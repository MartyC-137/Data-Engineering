...
Python script to read the data from SQL Server, transform it, and load it into 
Snowflake. I've modified this as a minimum reproducable example for the purposes of my 
project portfolio.
"""
...
# sql connection
cnxn = pyodbc.connect(
    Trusted_Connection="Yes", Driver="{SQL Server}", Server=server, Database=database
)
cursor = cnxn.cursor()

# stick your query inside the triple quotes
query = """select * from DATABASE.SCHEMA.EXCHANGERATES 
            where EXCHDATE > '2021-09-03' and EXCHDATE < '2021-09-09' 
...
df_sf[["FROM_CURRENCY", "TO_CURRENCY"]] = df_fx["EXCHANGE_ID"].str.split(
    "-", 1, expand=True
)
df_sf = df_sf[
    df_sf["TO_CURRENCY"].str.contains("AVG") is False
]  # drops rows that show avg - there are some GBP AVG
...
cursor.execute(
    f"""copy into CURRENCY_EXCHANGE_RATES(FROM_CURRENCY, 
                                            TO_CURRENCY, 
                                            EFFECTIVE_START, 
                                            EFFECTIVE_STOP, 
                                            RATE, 
...
cursor.execute('rm @MY_SCHEMA.FX_RATES pattern = ".*FX_RATES.*";')
